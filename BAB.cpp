...
void printAssignments(Node *min)
{
    ...
    cout << "Assign Job " << char(min->workerID + 'A')
         << " to Student " << (min->jobID + 1) << endl;
...
int main()
{
    ...
    for(int i=0;i<m;i++)
    {
      ...
      {
        scanf(" %d",&s[i][j]);
        costMatrix[i][s[i][j]-1] = costMatrix[i][s[i][j]-1] + (w2)*(j+1);
      }
    }
