...
int hungarian_imin(int a, int b) {
  return (a>b)?b:a;
}

void print_assign(int** C, int rows, int cols, int N){
  int orr = rows;
  int oc = cols;
  rows = hungarian_imin(orr,oc);
  int i,j;
  for(i=0;i<rows;i++)
  {
    for(j=0;j<N;j++)
    {
      if(C[i][j]==1)
      {
        fprintf(stderr, "Assign Job %d to Student %d\n", j+1, i+1);
        break;
      }
    }
  }
...
void print_allocation(hungarian_problem_t* p, int N) {
  print_assign(p->assignment, p->num_rows, p->num_cols, N) ;
}
